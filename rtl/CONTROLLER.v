...
module CONTROLLER(RESET, PHASE_CLK, P_UP, P_DOWN, 
    ...
    always@(posedge RESET or negedge PHASE_CLK) begin
        if (RESET)
            step <= 5'd8;
        else if (step > 7'd1 && POLARITY == 1'b1)
            step <= step >> 1;
    end
	...
	always@(posedge RESET or negedge PHASE_CLK) begin
		...
		else if (POLARITY) begin
			up_counter <= 3'd0;
			down_counter <= 3'd0;
		end
		else if (up_counter == 3'd4 || down_counter == 3'd4) begin
			up_counter <= 3'd0;
			down_counter <= 3'd0;
		end
		else if (P_UP == 1'b0 && FREQ_LOCK)
	...
	always@(posedge RESET or negedge PHASE_CLK) begin
		...
		else if (P_DOWN == 1'b0 && P_UP == 1'b1) begin
